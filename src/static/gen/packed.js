...
return defaultWriter.render(template,view,partials,config);};mustache.escape=escapeHtml;mustache.Scanner=Scanner;mustache.Context=Context;mustache.Writer=Writer;return mustache;})));console.log(Notification.permission);function show_notification(headline,body){note=new Notification(headline,{"body":body});}
const requestNotificationPermission=async()=>{const permission=await window.Notification.requestPermission();if(permission!=='granted'){throw new Error('Permission not granted for Notification');}}
const requestPermission=async()=>{const permission=await requestNotificationPermission();}
var socket=io();function renderTemplate(template_name,data){var template=document.getElementById(template_name).innerHTML;var rendered=Mustache.render(template,data);return rendered;}
...
const type=elem.getAttribute('type');if(!elem||elem===focused){continue;}
...
socket.on("notification",(data,callback)=>{show_notification(data["headline"],data["body"]);})
